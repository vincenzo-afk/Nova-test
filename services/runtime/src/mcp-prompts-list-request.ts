...
export class McpPromptsListRequestBuilder {
  private nextRequestId = 1;

  public create(options: Readonly<Record<string, unknown>> = {}): Result<McpPromptsListRequest> {
    if (!isRecord(options)) return err(this.error("MCP prompts/list request is malformed."));
    if (
      ...
      return err(this.error("MCP prompts/list cursor is invalid or too large."));
    }
