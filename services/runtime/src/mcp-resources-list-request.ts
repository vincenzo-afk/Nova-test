...
export class McpResourcesListRequestBuilder {
  private nextRequestId = 1;

  public create(options: Readonly<Record<string, unknown>> = {}): Result<McpResourcesListRequest> {
    if (!isRecord(options)) return err(this.error("MCP resources/list request is malformed."));
    if (
      ...
      return err(this.error("MCP resources/list cursor is invalid or too large."));
    }
