...
export class McpToolsListRequestBuilder {
  private nextRequestId = 1;

  public create(options: Readonly<Record<string, unknown>> = {}): Result<McpToolsListRequest> {
    if (!isRecord(options)) return err(this.error("MCP tools/list request is malformed."));
    if (
      ...
      return err(this.error("MCP tools/list cursor is invalid or too large."));
    }
