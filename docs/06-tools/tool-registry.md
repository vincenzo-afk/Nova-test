...
exposure: if one registry insertion fails, earlier insertions from that same
batch are removed before the error is returned. A validated MCP advertisement
