...
| **FM-19-009** | Plugin API compatibility mismatch | A plugin's declared NOVA API version range excludes the running NOVA API version at enable time. | Compatibility check rejects the manifest before the plugin becomes enabled. | Medium | Validate the declared compatibility range before sandboxed code loading. | Keep the plugin disabled and surface the required compatibility range so it can be updated or replaced. |
| **FM-19-010** | Plugin lifecycle state violation | A lifecycle operation is requested for a plugin that is not installed, already uninstalled, or in an invalid state for that operation. | Plugin Manager state transition guard rejects the request. | Low | Enforce the documented lifecycle state machine at every public operation. | Return a stable lifecycle error and leave the plugin state unchanged. |
| **FM-19-011** | Plugin sandbox provisioning failure | The isolated process environment cannot be provisioned before plugin code loading. | Sandbox provisioning callback or host boundary reports failure. | High | Provision and validate the sandbox before loading untrusted plugin code. | Keep the plugin failed/disabled, surface a bounded diagnostic, and do not load plugin code. |
