...
- The runtime now exposes a real `RuntimeTaskCoordinator` and `RuntimeApplication` composition root for Planner → Permission Manager → Executor → Verifier execution, correlated task-progress events, configuration, webhook, REST, and WebSocket services. Electron main now instantiates that shared runtime through `createDesktopRuntime`, and the renderer refreshes authoritative task status through the isolated preload bridge. Prisma/SQLite task checkpoints now persist creation and lifecycle mutations with crash recovery remapping; packaged desktop database-path migration, recovery startup wiring, and full capability registration remain subsequent integration work.
