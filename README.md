...
- The repository is currently version `0.1.0`. `pnpm install:windows` remains a non-destructive source-checkout bootstrap; `pnpm package:windows` now builds the per-user NSIS artifact, while service registration/unregistration, signing, and release publication remain separate explicit or hardening work.
